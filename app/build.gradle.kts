...
android {
    namespace = "com.ajstudioz.gallery"
    compileSdk = 36

    defaultConfig {
        applicationId = "com.ajstudioz.gallery"
        minSdk = 30
        targetSdk = 36
        versionCode = 32006
        versionName = "3.2.0"
    ...
    buildTypes {
        getByName("debug") {
            ...
            manifestPlaceholders["appProvider"] = "com.ajstudioz.gallery.debug.media_provider"
            buildConfigField("Boolean", "ALLOW_ALL_FILES_ACCESS", allowAllFilesAccess)
            buildConfigField(
                "String",
                "CONTENT_AUTHORITY",
                "\"com.ajstudioz.gallery.debug.media_provider\""
            )
        }
        getByName("release") {
    ...
    applicationVariants.all {
        outputs.forEach { output ->
            (output as BaseVariantOutputImpl).outputFileName =
                "AJ_Gallery-${versionName}-$versionCode-${productFlavors[0].name}" + mapsApiApplicationPrefix + ".apk"
        }
    }
}
